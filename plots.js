...
function buildBarChart(sample) {
  d3.json("samples.json").then((data) => {
    ...
    var result = resultArray[0];
    var top_ten_ids = result.otu_ids.slice(0, 10).map(numericIds => {
      return 'OTU ' + numericIds;
    }).reverse();
    var top_ten_values = result.sample_values.slice(0, 10).reverse();
  ...
  //build a bubble chart
    function buildBubbleChart(sample) {
      d3.json("samples.json").then((data) => {
        ...
        var values = result.sample_values.reverse();
